...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        @if (file_exists(public_path('build/manifest.json')) || file_exists(public_path('hot')))
            @vite(['resources/css/app.css', 'resources/js/app.js','resources/js/paneles.js','resources/js/add_panel.js','resources/js/filter_panel.js'])
        @else
            <style>
    ...
    <body class="w-auto h-auto bgcol2 pt-15">
        <x-nav></x-nav>
        <h1 class="col7 text-center pt-15 pb-15 text-4xl font-bold">
            Registros
        </h1>
                        ...
                        @if($record)
                            ...
                            @php
                                ...
                                switch($record->tipo_registro) {
                                    ...
                                    case 'transaction':
                                        // extra_info puede ser: 'ingreso', 'gasto', 'transferencia'
                                        if ($record->extra_info === 'ingreso') {
                                            ...
                                            $tipoCard = 'Gasto';
                                            $origen = ''; // Vacío para que no se renderice la línea de origen
                                    ...
                                    case 'debt':
                                        ...
                                        // 1. Origen: Mostramos el Monto Inicial original prestado
                                        $montoOriginal = '$' . number_format($record->monto_inicial, 2);
                                        $origen = 'Monto Inicial: ' . $montoOriginal;
                                        
                                        // 2. Formateamos la fecha de vencimiento que llega desde el controlador
                                        $vencimiento = !empty($record->vencimiento_registro) 
                                            ? date('d/m/Y', strtotime($record->vencimiento_registro)) 
                                            : 'Sin fecha';
                                            
                                        // 3. Destino: Concatenamos la prioridad (extra_info) y el vencimiento juntos
                                    ...
                                    case 'paymentGoal':
                                        ...
                                        // Mostramos el nombre de la meta asociada de forma segura
                                    ...
                                    case 'paymentDebt':
                                        ...
                                        // Mostramos el nombre de la deuda asociada de forma segura
                            ...
                            <x-col-home 
                                ...
                                :icono="$record->icono"
                                :monto="number_format($record->monto, 2)"
                                :origen="$origen"
                                :destino="$destino"
                                :categoria="$categoria"
